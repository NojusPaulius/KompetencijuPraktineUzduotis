...
exports.createComment = async (req, res) => {
    try {
        ...
        await creator.save();
        if (req.body.question) { // Changed from post to question
            const question = await Question.findById(req.body.question); // Changed from Post to Question
...
exports.deleteComment = async (req, res) => {
    try {
        ...
        // Get references to creator, question, and parent comment
        ...
        if (comment.comments.length > 0) {
            ...
            // No need to remove the comment from the question since it's just marked
            ...
            // Only now, remove the comment reference from the question's comments
            if (question) {
                const questionIndex = question.comments.indexOf(req.params.id);
                if (questionIndex > -1) {
                    question.comments.splice(questionIndex, 1);
                    await question.save(); // Save the question after removing the comment
                }
            }
        }
